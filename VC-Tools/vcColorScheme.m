function ColorList = vcColorScheme(varargin)
...
%   - Allows for an alternative base scheme.
% Updated: October 17, 2013 by Vasco Curdia
%   - It has balck in 3rd position, which lines well with the usual plot
%     settings, in which dash-dot is the third line style.
% 
...
if isShowSchemePlot && ~isLinePlot
    ...
    hold on
    for j=1:nColors
        fill([j-1,j-1,j,j]/nColors,[0,0.5+j/nColors/2,0.5+j/nColors/2,0],ColorList(j,:))
    end
    hold off
elseif  isShowSchemePlot && isLinePlot
    ...
    hold on
    for j=1:nColors
        plot(0:.1:1,j/(nColors+1)/3+(0:.1:1)*j/(nColors+1),'Color',ColorList(j,:),'LineWidth',LineWidth)
    end
    hold off
end
